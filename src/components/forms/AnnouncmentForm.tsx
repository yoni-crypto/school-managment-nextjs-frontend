...
const schema = z.object({
  ...
  birthDay: z.string().min(1, { message: "Birth Day is required" }), // Changed to string for compatibility with date input
  sex: z.enum(["male", "female"], { message: "Sex is required" }),
  img: z.instanceof(File, { message: "Image is required" }).optional(), // Optional to prevent issues if no file is uploaded
});

type Inputs = z.infer<typeof schema>;

interface AnnouncmentFormProps {
  type: "create" | "update";
  data?: Partial<Inputs>; // Use Partial<Inputs> to make all fields optional
}

const AnnouncmentForm: React.FC<AnnouncmentFormProps> = ({ type, data }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
      ...
      <div className="flex justify-between flex-wrap gap-4">
        ...
        <div className="flex flex-col gap-2 w-full md:w-1/4 justify-center">
          <label className="text-xs text-gray-400 flex items-center gap-2 cursor-pointer" htmlFor="img">
            <Image src="/icons8-upload-67.png" alt="" width={28} height={28} />
